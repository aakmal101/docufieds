...
export function SupportProfileMenu() {
    ...
    const handleLogout = async () => {
        try {
            ...
            window.location.href = '/auth/support-member/login'
        } catch (error) {
    ...
    if (loading) {
        return (
            <Button variant="ghost" className="relative h-10 w-10 rounded-full p-0 ring-2 ring-white ml-2">
                <Avatar className="h-10 w-10 bg-gray-100 animate-pulse"><AvatarFallback></AvatarFallback></Avatar>
            </Button>
        )
    }
                ...
                <div className="p-4 border-b bg-gray-50/50">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold text-gray-900 truncate">{user.fullName || 'Support Member'}</p>
                            {user.email && (
                                <p className="text-xs text-gray-500 truncate flex items-center gap-1">
                                    <Mail className="h-3 w-3" />{user.email}
                                </p>
                            )}
                            <Badge variant="outline" className="mt-1 bg-purple-50 text-purple-700 border-purple-200 text-[10px] px-1.5 py-0 h-4">Support Team</Badge>
                        </div>
                    </div>
                </div>
                <div className="p-1">
                    <DropdownMenuItem onClick={() => router.push('/admin/support-member/profile')} className="cursor-pointer">
                        <User className="mr-2 h-4 w-4" /><span>My Profile</span>
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => router.push('/admin/support-member/performance')} className="cursor-pointer">
                        <Activity className="mr-2 h-4 w-4" /><span>My Performance</span>
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => router.push('/admin/support-member/profile')} className="cursor-pointer">
                        <User className="mr-2 h-4 w-4" /><span>Update Profile</span>
                    </DropdownMenuItem>
                </div>
